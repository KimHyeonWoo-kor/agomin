import 'package:agomin_firebase/data/join_or_login.dart';
import 'package:agomin_firebase/helper/login_background.dart';
import 'package:agomin_firebase/screens/forget_pw.dart';
...
class AuthPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final Size size =
        MediaQuery
            .of(context)
            .size; // 미디어쿼리 라이브러리를 사용해서 핸드폰 화면 사이즈를 가져오는 것임
...
//                  Container(width: 100,height: 50,color:Colors.black,),
              ...
              builder: (context, joinOrLogin, child) =>
                  GestureDetector(
                      ...
                      child: Text(joinOrLogin.isJoin?"뒤로가기":"회원 가입",
                          style: TextStyle(fontSize: 20, color:joinOrLogin.isJoin?Colors.red:Colors.blue))),
            ),
  ...
  void _register(BuildContext context) async{
    ...
    if(user==null){
      final snackBar = SnackBar(content: Text('Pleas try again later.'),);
      ...
      child: SizedBox(
        ...
        child: Consumer<JoinOrLogin>(
          builder:(context, joinOrLogin, child) => RaisedButton(
            child: Text(
                joinOrLogin.isJoin?"join":"Login", style: TextStyle(fontSize: 20, color: Colors.white)),
